 ...
 *
 * THE HOOK IS THE UNBROKEN LINE. "Sixty-nine priests" is a statistic and
 * statistics slide off; an altar that has not gone a year unserved since 1697
 * is a shape, and it is what the register page is actually about. The copy
 * lives in i18n (`home.fathers*`) — read the note there before rewriting it.
 *
 * THE COPY NEVER COUNTS THE PORTRAITS. An earlier draft opened on how few
 * photographs survive; the parish is not advertising the gaps in its picture
 * collection. The row below carries itself and the prose stays silent about it.
 *
 * NO INVENTED FACES, here as on /priests. The row shows exactly the likenesses
 * the parish actually holds, read from the data rather than typed in, so a
 * portrait found tomorrow appears here the day it is added and nothing goes out
 * of step. The rest are represented by the counted rule beneath the row, never
 * by a placeholder head.
 */
...
// The year the parish's numbering starts. Derived so the "years unbroken"
// figure can never drift from the register or from the prose above it.
const FIRST_YEAR = 1697;
    ...
    <section className="parchment-sheen relative overflow-hidden bg-cream-dark section-padding" ref={sectionRef}>
      ...
      <div className="relative mx-auto max-w-5xl">
        <div ref={textRef} className="max-w-3xl">
          ...
          <h2 className="reveal-item font-serif text-[clamp(1.95rem,7.8vw,2.25rem)] leading-[1.07] text-navy md:text-5xl">
            {t.home.fathersTitle}
          </h2>
          <p className="reveal-item mt-7 text-[0.98rem] leading-relaxed text-text-muted md:text-lg">
            {t.home.fathersBody}
          </p>
          {/* Serif, so it goes UP on a phone while the sans line above it goes
              down — see the note in Patroness.tsx. */}
          <p className="reveal-item mt-5 font-serif text-[1.05rem] leading-relaxed text-navy/80 md:text-lg">
            {t.home.fathersBody2}
          </p>
        </div>
        ...
        <div ref={rowRef} className="mt-14 flex flex-wrap items-end gap-x-3 gap-y-5 sm:gap-x-4">
          ...
          {/* Those who left no likeness, drawn as the register draws them:
              a rule, not a face. */}
        ...
        {/* ── The three numbers ─────────────────────────────────────────────
            ...
            it is advertising. */}
        {/* `gap-3` on a phone: three columns inside a ~340px measure leaves
            about 97px each at the desktop gap, and the captions are small caps
            with tracking — they need the width more than the gutter does. */}
        <div className="mt-14 grid grid-cols-3 gap-3 border-y border-gold/25 py-8 sm:gap-6">
          {[
            { v: String(TERM_COUNT), k: t.home.fathersStat1 },
            { v: String(FIRST_YEAR), k: t.home.fathersStat2 },
            { v: String(THIS_YEAR - FIRST_YEAR), k: t.home.fathersStat3 },
          ].map((s) => (
            <div key={s.k}>
              <p className="font-display text-3xl leading-none text-navy tabular-nums md:text-4xl">
                {s.v}
              </p>
              <p className="ui-label mt-2.5 leading-snug text-text-muted">
                {s.k}
              </p>
            </div>
          ))}
        ...
        <div className="mt-10">
          <Link
            href="/priests"
            className="ui-label group inline-flex items-center gap-2 rounded-full bg-navy px-7 py-3.5 text-white transition-all duration-500 hover:bg-gold hover:text-navy"
          >
            {t.home.fathersCta}
